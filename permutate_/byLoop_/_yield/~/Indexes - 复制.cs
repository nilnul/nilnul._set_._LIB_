...
namespace nilnul.set.family.op_.of_.set_.permutate_.byLoop_._yield
{
	/*
	   ...
	   IEnumerable<int[]>
	{
		...
		{
			///Note: we keep lower numbers not move as possible as we could.
			...
			///
			/// we first find biggest number that: to itsRight (including itself), still has growth space.
			/// now how to grow (swap which tow?)? find the smallest growth.
			/// 
			/// we will move this number only one step, and fill the vacancy with smapplest number that can move left and so on (i.e., other numbers are reordered to initial state.)
			///
