...
namespace nilnul.set.op_.binary_
{
	...
	/// <summary>
	/// based on <see cref="nilnul.set.op_.unary_.IComplement{TEl}"/>. Analog to <see cref="op_.unary_.IComplement{TEl}"/>, take in an inner state of the Omega. The omega can be named any name besides omega if that's confusing, And the op is independent and in its own right; the inner state has nothing to do with operands, which might come from another universe.
	/// </summary>
...

	{
		...
		{
			var intersected = binary_.sect.cascader._CascadeX.Conjunct<TEl>(boxed.omega, par, par1);

			///remove the sets from the omega
